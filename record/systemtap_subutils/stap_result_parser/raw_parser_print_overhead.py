...
syscalls = {
    "read": (0, [{'int': 'fd'}, {'char*': 'buf'}, {'size_t': 'count'}, {'const char*': 'pathname'}]),
    "write": (1, [{'int': 'fd'}, {'char*': 'buf'}, {'size_t': 'count'}, {'const char*': 'pathname'}]),
    "open": (2, [{'const char*': 'pathname'}, {'int': 'flags'}, {'mode_t': 'mode'}]),
    "close": (3, [{'int': 'fd'}, {'const char*': 'pathname'}]),
    "stat": (4, [{'const char*': 'pathname'}, {'struct stat*': 'statbuf'}]),
    "fstat": (5, [{'int': 'fd'}, {'struct stat*': 'statbuf'}, {'const char*': 'pathname'}]),
    "lstat": (6, [{'const char*': 'pathname'}, {'struct stat*': 'statbuf'}]),
    "poll": (7, [{'struct pollfd*': 'fds'}, {'unsigned int': 'nfds'}, {'int': 'timeout'}]),
    "lseek": (8, [{'int': 'fd'}, {'off_t': 'offset'}, {'unsigned int': 'whence'}, {'const char*': 'pathname'}]),
    ...
    "rt_sigreturn": (15, []),
    "ioctl": (16, [{'int': 'fd'}, {'unsigned long': 'request'}, {'unsigned long': 'arg'}, {'const char*': 'pathname'}]),
    "pread64": (17, [{'int': 'fd'}, {'char*': 'buf'}, {'size_t': 'count'}, {'off_t': 'offset'}, {'const char*': 'pathname'}]),
    "pwrite64": (18, [{'int': 'fd'}, {'const char*': 'buf'}, {'size_t': 'count'}, {'off_t': 'offset'}, {'const char*': 'pathname'}]),
    "readv": (19, [{'int': 'fd'}, {'const struct iovec*': 'iov'}, {'int': 'iovcnt'}, {'const char*': 'pathname'}]),
    "writev": (20, [{'int': 'fd'}, {'const struct iovec*': 'iov'}, {'int': 'iovcnt'}, {'const char*': 'pathname'}]),
    ...
    "sendmsg": (46, [{'int': 'sockfd'}, {'struct msghdr*': 'msg'}, {'int': 'flags'}, {'const char*': 'pathname'}]),
    "recvmsg": (47, [{'int': 'sockfd'}, {'struct msghdr*': 'msg'}, {'int': 'flags'}, {'const char*': 'pathname'}]),
    ...
    "setsockopt": (54, [{'int': 'sockfd'}, {'int': 'level'}, {'int': 'optname'}, {'const void*': 'optval'}, {'int': 'optlen'}, {'const char*': 'pathname'}]),
    "getsockopt": (55, [{'int': 'sockfd'}, {'int': 'level'}, {'int': 'optname'}, {'char*': 'optval'}, {'int*': 'optlen'}, {'const char*': 'pathname'}]),
    ...
    "fcntl": (72, [{'int': 'fd'}, {'int': 'cmd'}, {'unsigned long': 'arg'}, {'const char*': 'pathname'}]),
    "flock": (73, [{'int': 'fd'}, {'int': 'operation'}, {'const char*': 'pathname'}]),
    "fsync": (74, [{'int': 'fd'}, {'const char*': 'pathname'}]),
    "fdatasync": (75, [{'int': 'fd'}, {'const char*': 'pathname'}]),
    "truncate": (76, [{'const char*': 'path'}, {'off_t': 'length'}]),
    "ftruncate": (77, [{'int': 'fd'}, {'off_t': 'length'}, {'const char*': 'pathname'}]),
    "getdents": (78, [{'int': 'fd'}, {'struct linux_dirent*': 'dirp'}, {'unsigned int': 'count'}, {'const char*': 'pathname'}]),
    "getcwd": (79, [{'char*': 'buf'}, {'size_t': 'size'}]),
    "chdir": (80, [{'const char*': 'path'}]),
    "fchdir": (81, [{'int': 'fd'}, {'const char*': 'pathname'}]),
    ...
    "fchmod": (91, [{'int': 'fd'}, {'mode_t': 'mode'}, {'const char*': 'pathname'}]),
    "chown": (92, [{'const char*': 'pathname'}, {'uid_t': 'owner'}, {'gid_t': 'group'}]),
    "fchown": (93, [{'int': 'fd'}, {'uid_t': 'owner'}, {'gid_t': 'group'}, {'const char*': 'pathname'}]),
    ...
    "fstatfs": (138, [{'int': 'fd'}, {'struct statfs*': 'buf'}, {'const char*': 'pathname'}]),
    ...
    "gettid": (186, []),
    "readahead": (187, [{'int': 'fd'}, {'off_t': 'offset'}, {'size_t': 'count'}, {'const char*': 'pathname'}]),
    ...
    "fsetxattr": (190, [{'int': 'fd'}, {'const char*': 'name'}, {'const void*': 'value'}, {'size_t': 'size'}, {'int': 'flags'}, {'const char*': 'pathname'}]),
    ...
    "fgetxattr": (193, [{'int': 'fd'}, {'const char*': 'name'}, {'void*': 'value'}, {'size_t': 'size'}, {'const char*': 'pathname'}]),
    "listxattr": (194, [{'const char*': 'path'}, {'char*': 'list'}, {'size_t': 'size'}]),
    "llistxattr": (195, [{'const char*': 'path'}, {'char*': 'list'}, {'size_t': 'size'}]),
    "flistxattr": (196, [{'int': 'fd'}, {'char*': 'list'}, {'size_t': 'size'}, {'const char*': 'pathname'}]),
    "removexattr": (197, [{'const char*': 'path'}, {'const char*': 'name'}]),
    "lremovexattr": (198, [{'const char*': 'path'}, {'const char*': 'name'}]),
    "fremovexattr": (199, [{'int': 'fd'}, {'const char*': 'name'}, {'const char*': 'pathname'}]),
    ...
    "getdents64": (217, [{'unsigned int': 'fd'}, {'struct linux_dirent64*': 'dirp'}, {'unsigned int': 'count'}, {'const char*': 'pathname'}]),
    "set_tid_address": (218, [{'int*': 'tidptr'}]),
    "restart_syscall": (219, []),
    "semtimedop": (220, [{'int': 'semid'}, {'struct sembuf*': 'sops'}, {'size_t': 'nsops'}, {'const struct timespec*': 'timeout'}]),
    "fadvise64": (221, [{'int': 'fd'}, {'off_t': 'offset'}, {'size_t': 'len'}, {'int': 'advice'}, {'const char*': 'pathname'}]),
    ...
    "epoll_ctl": (233, [{'int': 'epfd'}, {'int': 'op'}, {'int': 'fd'}, {'struct epoll_event*': 'event'}, {'const char*': 'pathname'}]),
    ...
    "inotify_init": (253, []),
    "inotify_add_watch": (254, [{'int': 'fd'}, {'const char*': 'pathname'}, {'u32': 'mask'}]),
    "inotify_rm_watch": (255, [{'int': 'fd'}, {'int': 'wd'}, {'const char*': 'pathname'}]),
    "migrate_pages": (256, [{'int': 'pid'}, {'unsigned long': 'maxnode'}, {'const unsigned long*': 'old_nodes'}, {'const unsigned long*': 'new_nodes'}]),
    "openat": (257, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'int': 'flags'}, {'mode_t': 'mode'}]),
    "mkdirat": (258, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'mode_t': 'mode'}]),
    "mknodat": (259, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'mode_t': 'mode'}, {'dev_t': 'dev'}]),
    "fchownat": (260, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'uid_t': 'owner'}, {'gid_t': 'group'}, {'int': 'flags'}]),
    "futimesat": (261, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'struct timeval*': 'times'}]),
    "newfstatat": (262, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'struct stat*': 'statbuf'}, {'int': 'flags'}]),
    "unlinkat": (263, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'int': 'flags'}]),
    ...
    "readlinkat": (267, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'char*': 'buf'}, {'int': 'bufsiz'}]),
    "fchmodat": (268, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'mode_t': 'mode'}, {'int': 'flags'}]),
    "faccessat": (269, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'int': 'mode'}, {'int': 'flags'}]),
    ...
    "sync_file_range": (277, [{'int': 'fd'}, {'off_t': 'offset'}, {'off_t': 'nbytes'}, {'unsigned int': 'flags'}, {'const char*': 'pathname'}]),
    "vmsplice": (278, [{'int': 'fd'}, {'const struct iovec*': 'iov'}, {'unsigned long': 'nr_segs'}, {'unsigned int': 'flags'}, {'const char*': 'pathname'}]),
    "move_pages": (279, [{'int': 'pid'}, {'unsigned long': 'count'}, {'const void**': 'pages'}, {'const int*': 'nodes'}, {'int*': 'status'}, {'int': 'flags'}]),
    "utimensat": (280, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'struct timespec*': 'times'}, {'int': 'flags'}]),
    ...
    "fallocate": (285, [{'int': 'fd'}, {'int': 'mode'}, {'off_t': 'offset'}, {'off_t': 'len'}, {'const char*': 'pathname'}]),
    ...
    "syncfs": (306, [{'int': 'fd'}, {'const char*': 'pathname'}]),
    "sendmmsg": (307, [{'int': 'sockfd'}, {'struct mmsghdr*': 'msgvec'}, {'unsigned int': 'vlen'}, {'int': 'flags'}, {'const char*': 'pathname'}]),
    "setns": (308, [{'int': 'fd'}, {'int': 'nstype'}, {'const char*': 'pathname'}]),
    ...
    "finit_module": (313, [{'int': 'fd'}, {'const char*': 'param_values'}, {'int': 'flags'}, {'const char*': 'pathname'}]),
    ...
    "stub_execveat": (322, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'const char*const*': 'argv'}, {'const char*const*': 'envp'}, {'int': 'flags'}]),
    ...
    "preadv2": (327, [{'int': 'fd'}, {'const struct iovec*': 'iov'}, {'unsigned long': 'iovcnt'}, {'unsigned long': 'pos_l'}, {'unsigned long': 'pos_h'}, {'int': 'flags'}, {'const char*': 'pathname'}]),
    "pwritev2": (328, [{'int': 'fd'}, {'const struct iovec*': 'iov'}, {'unsigned long': 'iovcnt'}, {'unsigned long': 'pos_l'}, {'unsigned long': 'pos_h'}, {'int': 'flags'}, {'const char*': 'pathname'}]),
    ...
    "statx": (332, [{'int': 'dirfd'}, {'const char*': 'pathname'}, {'int': 'flags'}, {'unsigned int': 'mask'}, {'struct statx*': 'statxbuf'}]),
    "specific_rread": (445, [{'struct rread_result*': 'result'}]),
    "specific_rread_diff": (446, [{'struct rread_result*': 'result'}]),
    "specific_timestamp_diff": (447, [{'unsigned long long*': 'diff_timestamp'}]),
    "specific_tsc_diff": (448, [{'unsigned long long*': 'diff_tsc'}]),
    "specific_instruction_diff": (449, [{'unsigned long long*': 'diff_instruction'}]),
    "specific_actual_cycle_diff": (450, [{'unsigned long long*': 'diff_actual_cycle'}]),
    "specific_cas_diff": (451, [{'unsigned int': 'box'}, {'unsigned int': 'channel'},{'unsigned int': 'type'}, {'unsigned long long*': 'diff_cas'}])
}

buf_related_syscall=["read", "write", "pread64", "pwrite64"]
already_include_pathname_syscalls = {
    "open": 0,
    "stat": 0,
    "lstat": 0,
    "access": 0,
...
def parse_item(raw, raw_type):
    ...
    if raw_type == TYPE_EXIT:
        ...
        buf_content = raw[IDX_SYS_EXIT.BUF_CONTENT.value]
        buf_arg_idx = 1 #replace exists value
        if buf_content != "NULL":
            args[buf_arg_idx] = buf_content

        path_name = raw[IDX_SYS_EXIT.PATH_NAME.value]
        if path_name != "NULL":
            args.append(path_name)

        retval = int(raw[IDX_SYS_EXIT.RETVAL.value])

        item["args"] = do_syscall_arg_type_casting(item["eventName"], args)
...
def do_syscall_arg_type_casting(evt_name, args):
    arg_format_list = get_syscall_arg_format(evt_name)

    # 1. pathname handling
    if evt_name in already_include_pathname_syscalls:
        pathname_idx = already_include_pathname_syscalls[evt_name]
        str_parsed_pathname_idx = 6
        args[pathname_idx] = str(args[str_parsed_pathname_idx])
        del args[str_parsed_pathname_idx]

    # 2. remove dummy arg
    goal_arg_len = len(arg_format_list)
    tmp_args = []
    if len(args) == 7: # it has custom pathname
        for i in range(goal_arg_len - 1):
            tmp_args.append(args[i])
        tmp_args.append(args[6]) # append pathname
    else:
        if goal_arg_len == 7: # if failed to finding pathname
            goal_arg_len -= 1
        for i in range(goal_arg_len):
            tmp_args.append(args[i])
    args = tmp_args
    ...
    for idx in range(len(args)):
        ...
        try:
            if args[idx]["value"] == "":
                pass
            elif (arg_name == "pathname" or evt_name in buf_related_syscall) and  evt_name != "fanotify_mark":
                args[idx]["value"] = str(args[idx]["value"]) # str
            elif data_type == "int":
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', "--time", dest="time", required=False, type=int, help="time limit(s)")
