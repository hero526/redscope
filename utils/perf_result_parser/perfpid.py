...
def get_client_info(target_db_name, target_col_name):
    ...
    if len(client_info) != 1:
        print(f"Cannot find arrival rate and # request! ({target_col_name})")
        return (-1, -1)
    else:
...
if __name__ == "__main__":
    ...
    print("database,operation,client,interval,arrivalRate,requestCount,iteration,eventName,value")
    ...
    for collection in colList:
        ...
        for row in data:
            ...
            if row["value"] == "<not counted>":
                ...
                value = row["value"]
            print(f'{database},{operation},{client},{interval},{arrival_rate},{req_count},{iteration},{metric},{value}')
